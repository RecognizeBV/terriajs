...
GlobeOrMap.prototype._highlightFeature = function(feature) {
    ...
    if (defined(feature)) {
        ...
        if (defined(feature.cesium3DTileFeature)) {
            hasGeometry = true;

            var originalColor = feature.cesium3DTileFeature.color;

            feature.cesium3DTileFeature.color = Color.fromCssColorString('#08ABD5');

            this._removeHighlightCallback = function() {
                feature.cesium3DTileFeature.color = originalColor;
            };
        }
