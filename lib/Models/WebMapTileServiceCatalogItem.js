...
/**
 * Returns the first matrixSetID and matrixSet or null if none were found
 * @param layer
 * @param capabilities
 * @returns {*}
 */
WebMapTileServiceCatalogItem.getTileMatrixSet = function(layer, capabilities) {
    ...
    for (var i = 0; i < tileMatrixSetLinks.length; ++i) {
        ...
        if (capabilities.usableTileMatrixSets[set]) {
            return [set, capabilities.usableTileMatrixSets[set]];
        }
    }
    return null;
};
...
WebMapTileServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
    ...
    var firstSupportedMatrixSet = WebMapTileServiceCatalogItem.getTileMatrixSet(thisLayer, capabilities);
    if (firstSupportedMatrixSet !== null) {
        [tileMatrixSetID, tileMatrixSetLabels] = firstSupportedMatrixSet;
    }
...
WebMapTileServiceCatalogItem.capabilitiesXmlToJson = function(xml) {
    ...
    for (var i = 0; i < matrixSets.length; ++i) {
        ...
        if (matrixSet.SupportedCRS !== 'urn:ogc:def:crs:EPSG::900913'
